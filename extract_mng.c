...
mng_bool myreaddata(mng_handle hMNG, mng_ptr pBuf, mng_uint32 iSize,
                    mng_uint32 *iRead) { /* get to my file handle */
...
mng_bool myiterchunk(mng_handle hMNG, mng_handle hChunk, mng_chunkid iChunktype,
                     mng_uint32 iChunkseq) { /* get to my file handle */
  userdatap pMydata = (userdatap)mng_get_userdata(hMNG);
  char aCh[4];
  char zIndent[80];
  int iX;
  ...
  if (iChunktype == MNG_UINT_IHDR) {
    ...
    mng_getchunk_ihdr(hMNG, hChunk, &width, &height, &bitDepth, &colorType,
...
int dumptree(char *zFilename, char *fileprefix) {
  ...
  if ((pMydata->hFile = fopen(zFilename, "rb")) ==
      NULL) { /* error out if we can't */
...
int main(int argc, char *argv[]) {
  ...
  if (argc > 1) /* need that first parameter ! */
    ...
    printf("\nUsage: extract_mng <file.mng> <prefix>\n\n");
